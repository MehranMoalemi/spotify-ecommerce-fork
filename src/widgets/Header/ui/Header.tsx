...
export const Header = () => {
  ...
  const defaultPath: '/' | 'contactus' | 'packages' = validPaths.includes(path as any)
    ? (path as '/' | 'contactus' | 'packages')
    : '/';
  ...
  return (
    <header className={`desktop_nav relative md:fixed right-0 left-0 h-24 ${isLightsVisible?null:'z-50'}  flex justify-between w-full py-8 max-w-[80rem] px-10 lg:px-24 mx-auto`}>
      ...
      {/* Navigation */}
      <nav
        className={`mobile_nav fixed top-0 left-0 right-0 md:relative h-screen md:h-auto  md:bg-transparent z-50 transition-all duration-300 ease-in-out transform ${isNavOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'
